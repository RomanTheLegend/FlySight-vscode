...
typedef struct
{
  ...
  APP_BLE_ConnStatus_t EndDevice_Connection_Status[1];
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
  {
    ...
    if ((BleApplicationContext.Device_Connection_Status == APP_BLE_FAST_ADV) || (BleApplicationContext.Device_Connection_Status == APP_BLE_LP_ADV))
    {
  ...
  case HCI_LE_META_EVT_CODE:
  {
    ...
    case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
    {
       ...
       */
      int conn_int = p_connection_complete_event->Conn_Interval;

      APP_DBG_MSG(">>== HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE - Connection handle: 0x%x\n\r", p_connection_complete_event->Connection_Handle);
      APP_DBG_MSG("     - Connection established with peer: @:%02x:%02x:%02x:%02x:%02x:%02x\n\r",
      ...
      APP_DBG_MSG("     - Connection Interval:   %d ms\n\r     - Connection latency:    %d\n\r     - Supervision Timeout: %d ms\n\r",
    		  	  conn_int * 1.25f,
    ...
    if (event_type == ADV_IND)
    {
      ...
      while (k < event_data_size)
      {
        ...
        case 0x09: /* now get local name */
          ...
          if (adlength >= 6 && !memcmp(&adv_report_data[k + 2], "ENGO 2", 6))
          {
  ...
  case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
  {
    ...
    case ACI_GAP_PROC_COMPLETE_VSEVT_CODE:
    {
      ...
      /* USER CODE BEGIN EVT_BLUE_GAP_PROCEDURE_COMPLETE */
      {
        ...
        if (p_gap_evt_proc_complete->Procedure_Code == GAP_GENERAL_DISCOVERY_PROC && p_gap_evt_proc_complete->Status == 0x00)
        {
          APP_DBG_MSG("-- GAP GENERAL DISCOVERY PROCEDURE_COMPLETED\n\r");
          /*if a device found, connect to it, device 1 being chosen first if both found*/
          if (BleApplicationContext.EndDevice1Found == 0x01 && BleApplicationContext.EndDevice_Connection_Status[0] != APP_BLE_CONNECTED_CLIENT)
          {
            APP_DBG_MSG("-- ENGO 2 detected\n\r");
    ...
    case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
    {
      ...
      {
        APP_DBG_MSG("     - Pairing KO \n     - Status: 0x%x\n     - Reason: 0x%x\n\r", p_pairing_complete->Status, p_pairing_complete->Reason);
      }
      else
      {
    ...
    case ACI_GATT_ERROR_RESP_VSEVT_CODE:
    {
      ...
      APP_DBG_MSG("!!! ACI_GATT_ERROR_RESP_VSEVT_CODE 0x%x Connection_Handle: 0x%x Req_Opcode 0x%x Attribute_Handle: 0x%x\n\r",
    		  p_pr->Error_Code, p_pr->Connection_Handle,p_pr->Req_Opcode,p_pr->Attribute_Handle);
...
//      APP_DBG_MSG(" DEFAULT DEBUG_EVT_CODE 0x%x\n\r", p_blecore_evt->ecode);
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  else
  {
    ...
    APP_DBG_MSG("  Public Bluetooth Address: %02x:%02x:%02x:%02x:%02x:%02x\n\r", p_bd_addr[5], p_bd_addr[4], p_bd_addr[3], p_bd_addr[2], p_bd_addr[1], p_bd_addr[0]);
  }
...
static void ConnReq1(void)
{
  ...
  uint16_t handleArray[8];


  aci_hal_get_link_status(connStatus, handleArray);
